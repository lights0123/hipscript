  ...
  case spv::OpExtInst:
    if (inst->ext_inst_type == SPV_EXT_INST_TYPE_NONSEMANTIC_CLSPVREFLECTION) {
      ...
      case clspv::reflection::ExtInstKernel: {
        ...
        auto kernel_flags = constants[inst->words[inst->operands[7].offset]];
        strings[inst->result_id] = name;
        *str << "kernel_decl," << name << ",printf,"
             << !!(kernel_flags &
                   clspv::reflection::ExtKernelPropertyFlags::MayUsePrintf)
             << "\n";
        break;
      }
      ...
      case clspv::reflection::ExtInstPrintfInfo: {
        // Emit constant data entry.
        auto printf_id = constants[inst->words[inst->operands[4].offset]];
        const auto &format = strings[inst->words[inst->operands[5].offset]];
        *str << "printf,id," << printf_id << ",format,";
        ...
        *str << ",args,";
        bool first = true;
        for (size_t i = 6; i < inst->num_operands; i++) {
          if (!first)
            *str << ';';
          *str << constants[inst->words[inst->operands[i].offset]];
          first = false;
        }
        *str << '\n';
        break;
